...
export class AddressSuggestionTools {
  ...
  static async addressMatchesOneOrMoreSuggestions(address: string, addressTypesToFilterBy?: AddressTypes[]): Promise<boolean> {
    const apiResponse = await AddressSuggestionTools.getApiAddressSuggestions(address);
    ...
    if (apiResponse) {
      ...
      let addressSuggestions: string[] = apiResponse.map(address => address.FullAddress);

      return _.includes(addressSuggestions, address);
    }

    return false;
  }

  static async getApiAddressSuggestions(query: string) {
    if (!this.bearerToken) {
