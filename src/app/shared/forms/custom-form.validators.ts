...
export class CustomFormValidators extends Validators {
  ...
  static doesNotContainThis = (target: string, errorMessage?: string) => (
    ...
    if (text.value) {
      ...
      return includes(text.value, target)
  ...
  static isNumeric(control: FormControl): IErrorMessage {
    ...
    return isNaN(control.value)
      ? { response: "Oops, only numbers here" }
      : null;
  }
  ...
  static noDecimals(control: FormControl) {
    ...
    return control.value.indexOf(".") !== -1
      ? { response: "Oops, we can't take decimals here" }
      : null;
  }
  ...
  static range = (min: number, max: number) => (text: FormControl) => {
    ...
    return valid
      ? null
      : { response: "VALUE_MUST_BE_IN_RANGE", params: { min, max } };
  };

  static minLength = (length: number, errorMessage?: string) => (
    text: FormControl
  ) => {
    if (!text.value) {
      return;
    ...
    const response =
      errorMessage || `Oops, we need at least ${length} characters here`;
    return text.value.length < length ? { response, params: { length } } : null;
  };

  static maxLength = (length: number, errorMessage?: string) => (
    text: FormControl
  ) => {
    if (!text.value) {
      return;
    ...
    const response =
      errorMessage || `Oops, we need a max of ${length} characters here`;
    return text.value.length > length ? { response, params: { length } } : null;
  };
  ...
  static isNoQuotesForwardSlashQuestionMarkOrSpaces(
    control: FormControl
  ): IErrorMessage {
    if (control.value) {
      return CustomFormValidators.noQuotesForwardSlashQuestionMarkOrSpaces(
        control.value
      )
        ? null
